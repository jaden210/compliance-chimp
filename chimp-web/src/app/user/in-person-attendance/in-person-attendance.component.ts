...
import { SignatureBottomSheetComponent } from "../survey/signature-bottom-sheet.component";
...
@Component({
  standalone: true,
  selector: "app-user-in-person-attendance",
  templateUrl:
    "../../account/training/in-person-attendance/in-person-attendance.component.html",
  styleUrls: [
    "../../account/training/in-person-attendance/in-person-attendance.component.scss",
  ],
  imports: [
    CommonModule,
    FormsModule,
...
export class UserInPersonAttendanceComponent implements OnInit, OnDestroy {
  ...
  backButtonLabel = 'Back to Training';
  ...
  private loadSurvey(surveyId: string, teamMembers: TeamMember[]): void {
    this.userService
      ...
      .subscribe((survey) => {
        ...
        this.attendees = (survey.trainees || []).map(
          (tmId: string, index: number) => {
            const tm = teamMembers.find((m) => m.id === tmId);
            return {
              teamMember: tm ||
                ({ id: tmId, name: "Unknown", teamId: survey.teamId } as any),
          ...
          .subscribe((responses) => {
            this.attendees.forEach((a) => {
              const resp = responses.find(
                (r) => r.teamMemberId === a.teamMember.id
              );
              if (resp) {
                a.signed = true;
                a.response = resp;
  ...
  submitResponse(attendee: AttendeeRow): void {
    ...
    sheetRef
      .afterDismissed()
      .subscribe((signatureDataUrl: string | undefined) => {
  ...
  private async saveResponse(
    ...
    try {
      ...
      // Get manager ID from currentManager or teamMember (pseudo-member created for managers)
      const managerId =
        this.userService.currentManager?.id ||
        this.userService.teamMember?.id;
      if (managerId) {
        response.collectedBy = managerId;
      }

      await this.userService.createResponse(response);
      ...
      this.snackBar.open(
        `Signature collected for ${attendee.teamMember.name}`,
        "Close",
        { duration: 3000 }
      );
  ...
  goBack(): void {
    // Navigate back within the Banana Handbook
    window.history.back();
  }
